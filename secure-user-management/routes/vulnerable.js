...
if (document.getElementById("logout-button") && document.getElementById("admin-view-button")) {
  ...
  document.getElementById("admin-view-button").addEventListener("click", async () => {
    ...
    if (adminPanel.style.display === "none" || !adminPanel.style.display) {
      try {
        // VULNERABLE: Uses the vulnerable admin check endpoint that doesn't properly verify roles
        const response = await fetch(`/api/vulnerable/admin-check?sessionId=${sessionId}`)
        const data = await response.json()

        // This will always show the admin panel if the user has a valid session
        // due to the vulnerability in the server-side admin check
  ...
  document.getElementById("logout-button").addEventListener("click", () => {
    // Vulnerable: No server-side session cleanup, just client-side removal
